...
1. Builds and runs the AppKit vertical example and Visual Haskell native interaction/resource tests.
...
This proves consistent build metadata and execution on the current host. It does not emulate the requested macOS release. Before HaskeLUI publishes a production support floor, the same target-built native suite must run on that oldest release in a maintained physical, virtual, or CI environment.
