...
`uih-textmate` will load declarative TextMate grammars and themes at runtime, tokenize edited documents, and convert the result into UIH's generic revision-bound text presentation layers.

The package belongs outside `uih-core`. Core owns portable text ranges, styles, attributed content, and presentation layers. This package will own TextMate-specific manifests, grammar rules, scope stacks, theme selectors, regex execution, line-state caching, and compatibility behavior.
...
- Putting TextMate scopes, selectors, or grammar types into `uih-core`.
...
│   └── UIH.hs                   # future: conversion to TextLayer
├── cbits/                       # future: small prefixed Oniguruma C bridge
└── test/                        # future: fixtures and differential tests
```

`uih-core` must not depend on this package. `uih-textmate` may depend on `uih-core` in its UIH adapter because the dependency direction remains one-way.
...
- Expose a small UIH-prefixed C ABI rather than binding the complete library directly.
...
Native Oniguruma reports byte offsets for UTF-8 input. Each line cache therefore needs a validated boundary index capable of converting UTF-8 byte offsets to UIH Unicode scalar offsets. The AppKit backend will continue converting UIH scalar offsets to UTF-16; TextMate-specific units must not leak into Core.
...
- Converting token spans into UIH styles and layers.
...
Editor selection, IME, search results, diagnostics, and other UIH presentation layers remain independent and may overlay theme output property-by-property.

VS Code workbench colors and semantic-token theme rules are outside the initial TextMate theme contract. They can be modeled later by separate UIH theme and semantic-token adapters.
...
The first release should load user-supplied or application-explicit grammar directories and ship only small test fixtures written for UIH. A curated built-in grammar collection should be a later, separately reviewed distribution decision with machine-readable provenance, version pins, licenses, and notices.

Loading an extension already present on a user's machine does not by itself authorize UIH to redistribute that extension.
...
Development tests invoke a pinned `vscode-textmate` plus `vscode-oniguruma` tool outside the shipping application. Given identical grammar, theme, and input, it records per-line scopes and rule-state behavior. UIH output is normalized to the same index convention and compared.
...
### Phase 2: manifests, themes, and UIH adapter
...
Recommendation: start with explicit user/application grammar directories and UIH-owned fixtures. Design a curated bundle only after the engine is stable and provenance automation exists.
...
The TextMate package could emit semantic scope spans and leave all theme resolution to the application, or it could load themes and emit final UIH styles. The first is more composable; the second is much easier for applications and necessary for VS Code theme reuse.
