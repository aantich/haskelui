...
module HaskeLUI.Property.Spike
  ( Action
  , ApplyError (..)
  , Document (..)
  , ElementProperty
...
-- | The small van Laarhoven core required by HaskeLUI. It is structurally
-- compatible with lenses produced by generic-lens and the lens ecosystem,
-- without requiring the full lens package in haskelui-core.
...
-- | HaskeLUI's reified action language can retain property identity while using
